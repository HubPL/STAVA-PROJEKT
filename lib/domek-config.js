...
export const DOMEK_INFO = {
  ...
  // Opisy
  opisKrotki: "Oferujemy trzy domy całoroczne w stylu nowoczesnej stodoły o powierzchni 67 m²",
  
  opis: `Oferujemy trzy domy całoroczne w stylu nowoczesnej stodoły o powierzchni 67 m²,
przeznaczone do wypoczynku dla 4 osób, z opcją do 6 osób.

Każdy z domów posiada dwie sypialnie dwuosobowe, usytuowane na piętrze, oraz salon
kominkowy na parterze z rozkładaną sofą i stołem jadalnym dla czterech osób. Wszystkie
pokoje są klimatyzowane.

Goście do dyspozycji mają własną strefę wypoczynku, obejmującą tarasy zewnętrzne,
na których znajdują się zestawy wypoczynkowe, grill oraz wanna opalana drewnem
do relaksacyjnych kąpieli w bezpośredniej bliskości z naturą.

W obrębie strefy wypoczynku dostępna jest również sauna fińska z pomieszczeniem
relaksacyjnym z widokiem na las oraz zewnętrzną beczką do morsowania.

Na terenie znajdują się dwa stawy rybne dla miłośników wędkarstwa oraz wyznaczone
miejsce na ognisko.`,
...
// Helper: czy dana data to weekend (piątek, sobota = noc weekendowa)
export const czyWeekend = (date) => {
  const d = new Date(date);
  const dzien = d.getDay(); // 0=niedziela, 5=piątek, 6=sobota
  return dzien === 5 || dzien === 6; // noc z piątku na sobotę i z soboty na niedzielę
};
...
export const getAktualnaCena = (config) => {
  ...
  if (cenaSezonu) {
    // Zwróć cenę weekday jako bazową (niższą)
    return cenaSezonu.cena_weekday || cenaSezonu.cena || config.ceny.podstawowa;
  }

  return config.ceny.podstawowa;
};

// Nowa funkcja do pobierania ceny dla konkretnej daty (z uwzględnieniem weekend/dzień powszedni)
export const getCenaWDniu = (config, data) => {
  ...
  const weekend = czyWeekend(sprawdzanaData);
  ...
  if (cenaSezonu) {
    if (weekend) {
      return cenaSezonu.cena_weekend || cenaSezonu.cena || config.ceny.podstawowa_weekend || config.ceny.podstawowa;
    }
    return cenaSezonu.cena_weekday || cenaSezonu.cena || config.ceny.podstawowa;
  }

  // Cena podstawowa z uwzględnieniem weekendu
  if (weekend) {
    return config.ceny.podstawowa_weekend || config.ceny.podstawowa;
  }
  return config.ceny.podstawowa;
};
...
// Nowa funkcja do szczegółowego rozliczenia terminów z różnymi sezonami i weekend/weekday
export const getRozliczenieSezonowe = (config, dataOd, dataDo) => {
  ...
  // Grupuj dni według cen/sezonów i weekend/weekday
  const grupy = {};

  dni.forEach(dzien => {
    const cena = getCenaWDniu(config, dzien);
    const weekend = czyWeekend(dzien);
    ...
    const nazwaSezon = sezon ? sezon.nazwa : 'Cena podstawowa';
    const typDnia = weekend ? 'weekend' : 'dzień powszedni';
    const klucz = `${nazwaSezon}_${typDnia}_${cena}`;
...
// Nowa funkcja do kalkulacji ceny z uwzględnieniem liczby osób, zwierząt i konkretnych dat z sezonami
export const kalkulujCeneZOsobami = (config, liczbOsob, liczbaNocy = 1, dataOd = null, dataDo = null, czyMaZwierze = false) => {
  if (!config?.ceny) return null;

  const bazowaLiczbaOsob = config.bazowa_liczba_osob || 4;
  const cenaZaDodatkowaOsoba = config.ceny.cena_za_dodatkowa_osoba || 0;
  const cenaZaZwierze = config.ceny.cena_za_zwierze || 0;
  const dodatkoweOsoby = Math.max(0, liczbOsob - bazowaLiczbaOsob);
  const oplataDobowa = (dodatkoweOsoby * cenaZaDodatkowaOsoba) + (czyMaZwierze ? cenaZaZwierze : 0);
  ...
  if (dataOd && dataDo) {
    ...
    if (!rozliczenie || rozliczenie.length === 0) {
      ...
      const cenaZaDobe = cenaBazowa + oplataDobowa;
      ...
      return {
        ...
        czyMaZwierze,
        cenaZaZwierze: czyMaZwierze ? cenaZaZwierze : 0,
    ...
    const szczegoly = rozliczenie.map(grupa => {
      ...
      const oplataDodobaGrupy = oplataZaDodatkoweOsobyGrupy + (czyMaZwierze ? cenaZaZwierze : 0);
      const cenaZaDobeGrupy = cenaBazowaGrupy + oplataDodobaGrupy;
      ...
      return {
        nazwa: grupa.nazwa,
        typDnia: grupa.typDnia || null,
        czyWeekend: grupa.czyWeekend || false,
        cena: cenaBazowaGrupy,
        dni: grupa.dni,
        oplataZaDodatkoweOsoby: oplataZaDodatkoweOsobyGrupy,
        oplatZaZwierze: czyMaZwierze ? cenaZaZwierze : 0,
...
export const formatujSzczegolyCeny = (obliczenia) => {
  ...
  if (obliczenia.rozliczenieSezonowe && obliczenia.rozliczenieSezonowe.length > 0) {
    obliczenia.rozliczenieSezonowe.forEach(sezon => {
      const typDniaLabel = sezon.typDnia ? ` (${sezon.typDnia})` : '';
      szczegoly.push({
        opis: `${sezon.nazwa}${typDniaLabel} - ${sezon.dni} ${sezon.dni === 1 ? 'noc' : 'nocy'} × ${sezon.cena} PLN`,
      ...
      if (sezon.oplatZaZwierze > 0) {
        szczegoly.push({
          opis: `+ zwierzę × ${sezon.dni} ${sezon.dni === 1 ? 'noc' : 'nocy'} × ${sezon.oplatZaZwierze} PLN`,
          cena: sezon.oplatZaZwierze * sezon.dni,
          typ: 'zwierze'
        });
      }
    });
    ...
    if (obliczenia.czyMaZwierze && obliczenia.cenaZaZwierze > 0) {
      szczegoly.push({
        opis: `Opłata za zwierzę × ${obliczenia.cenaZaZwierze} PLN`,
        cena: obliczenia.cenaZaZwierze,
        typ: 'zwierze'
      });
    }
  }
