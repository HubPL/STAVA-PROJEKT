 ...
 * @returns {Promise<Object>} Wynik operacji z liczbą usuniętych rezerwacji
 */
export const cleanupExpiredReservations = async () => {
  try {
    ...
    // Usuń przeterminowane rezerwacje
    let deletedCount = 0;
    const deletePromises = expiredReservations.map(async (reservation) => {
      try {
        ...
        console.error(`Błąd usuwania rezerwacji ${reservation.id}:`, deleteError);
        return { success: false, id: reservation.id, error: deleteError };
      }
    });
    ...
    const result = {
      ...
      failedDeletes: deleteResults.filter(r => !r.success),
      message: `Znaleziono ${expiredReservations.length} przeterminowanych rezerwacji, usunięto ${deletedCount}`
    };
    ...
    console.error('Błąd podczas czyszczenia przeterminowanych rezerwacji:', error);
    return {
      success: false,
      error: error.message,
      foundExpired: 0,
      deletedCount: 0,
      message: `Błąd podczas czyszczenia: ${error.message}`
    };
  }
}; 