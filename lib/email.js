...
// Funkcja pomocnicza do konwersji dat z uwzględnieniem strefy czasowej (Europe/Warsaw)
const formatDateForEmail = (dateInput) => {
  ...
  if (dateInput?.toDate) {
    ...
    // Aby uniknąć przesunięć o 1 dzień (DST/UTC), ustawiamy południe w UTC
...
function generateGuestConfirmationHTML(rezerwacja) {
  ...
  if (rezerwacja.selectedDomki && Array.isArray(rezerwacja.selectedDomki)) {
    ...
    rezerwacja.selectedDomki.forEach((domek, index) => {
      ...
      const zwierzeInfo = domek.czyMaZwierze ? ' | <strong>Zwierzę:</strong> Tak' : '';

      domkiDetailsHTML += `
        <div style="margin-bottom: 15px; ${index > 0 ? 'border-top: 1px solid #e0e0e0; padding-top: 15px;' : ''}">
          <h4>Domek ${domek.domekId.replace('D', '')}</h4>
          <p><strong>Osoby:</strong> ${domek.liczbOsob} | <strong>Noce:</strong> ${domek.iloscNocy} | <strong>Cena:</strong> ${domek.cenaCałkowita || domek.cenaCalkowitaDomku} PLN${zwierzeInfo}</p>
